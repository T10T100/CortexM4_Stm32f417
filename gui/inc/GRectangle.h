#ifndef GRECTANGLE_CLASS
#define GRECTANGLE_CLASS

#include "GcontentPaneInterface.h"

template <typename Color>
class GRectangle : public virtual GPaneInterface<Color> {
    private :
        template <typename D>
        int32_t fillRectPart (D rect, Color color)
        {
            register int32_t x = rect.x;
            register int32_t y = rect.y;
            register int32_t h = this->h;
            register int32_t S = h * x;
            
            for (int32_t __x = S + h * rect.w; __x > S; __x -= h) {
                for (uint32_t __y = rect.h + y - 1; __y > y; __y--) {
                    this->frame[__y + __x] = color;
                }
            }
            return 0;
        }
    
    public :
        GRectangle (GraphicFrame<Color, MaxGuiRange> *frame)
        {
            GPaneInterface<Color>::operator () (frame);
        }
        template <typename D>
        int32_t rectangle (D r, uint32_t width, Color color = 0xffff)
        {
            D b0 = {r.x, r.y, r.w, width};
            D b1 = {r.x + r.w - width, r.y, width, r.h};
            D b2 = {r.x, r.y + r.h - width, r.w, width};
            D b3 = {r.x, r.y, width, r.h};
            fillRectPart(b0, color);
            fillRectPart(b1, color);
            fillRectPart(b2, color);
            fillRectPart(b3, color);
            
            return 0;
        }

        template <typename D>
        int32_t DrawHatched (D rect, Color color, uint8_t hatch)
        {
            
            return 0;
        }
};    




#endif

/*End of file*/

